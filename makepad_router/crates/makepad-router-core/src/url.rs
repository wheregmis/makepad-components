...
pub fn parse_query_map(query: &str) -> HashMap<String, String> {
    ...
    let pair_count = q.bytes().filter(|&b| b == b'&').count() + 1;
    // Optimization: route query parsing sits on the navigation path.
    // Pre-size the map from the raw `&` count so typical multi-key URLs avoid
    // HashMap growth and rehashing while we decode each component.
    let mut out = HashMap::with_capacity(pair_count);
    for pair in q.split('&') {
        if pair.is_empty() {
            continue;
        }
...
fn decode_www_form_component(input: &str) -> String {
    // Optimization: most router query keys/values are plain ASCII with no `%xx`
    // escapes or `+` spaces. Return a direct String copy in that common case so
    // we skip the temporary byte buffer and UTF-8 reconstruction work.
...
mod tests {
    ...
    fn parse_query_map_fast_path_benchmark() {
        ...
        const ITERATIONS: usize = 200_000;
        const QUERY: &str = "?tab=active&region=ca&view=table&scope=team";

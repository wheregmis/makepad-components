...
#[test]
fn popover_page_syncs_status_labels_on_apply() {
    let source = include_str!("../src/ui/popover_page.rs");

    assert!(
        source.contains("impl ScriptHook for GalleryPopoverPage"),
        "popover page should sync its status labels during apply"
    );
    assert!(
        source.contains("vm.with_cx_mut(|cx| self.sync_status_labels(cx));"),
        "popover page should refresh both status labels after apply"
    );
}

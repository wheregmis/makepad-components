...
const ICON_RESOURCE_PREFIX: &str = "makepad-icon:resources/icons/";
...
/// Returns generated icon entries as `(widget_name, icon_stem, icon_file_name)` tuples.
fn icon_entries(icons_dir: &Path) -> std::io::Result<Vec<(String, String, String)>> {
    ...
    for path in paths {
        ...
        if let Some(widget_name) = to_widget_name(stem) {
            entries.push((widget_name, stem.to_string(), file_name.to_string()));
        }
    }
    Ok(entries)
}

fn generate_preview(entries: &[(String, String, String)]) -> String {
    ...
    for (widget_name, label, file_name) in entries {
        output.push_str(&format!(
            "    IconGalleryEntry{{icon_name: \"{label}\", widget_name: \"{widget_name}\", resource_path: \"{ICON_RESOURCE_PREFIX}{file_name}\"}},\n"
        ));
    }
    output.push_str("];\n");
    output

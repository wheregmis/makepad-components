...
20. **Wrapper widgets must expose the live props the DSL sets**: If you replace a plain widget instance with a registered wrapper (`mod.widgets.MyWidgetBase = #(MyWidget::register_widget(vm))`), every property used on that wrapper in `script_mod!` must exist as a `#[live]` Rust field or be handled explicitly in `ScriptHook`. Otherwise runtime apply will fail with `property X not defined on type`. Example: if the DSL writes `ShadPaginationPageButton{text: "1"}`, the wrapper must declare `#[live] text: ArcStringMut` and synchronize that value into its inner children.

21. **Slot-reused wrapper widgets must reset transient inner state**: If a wrapper reuses inner buttons/views across different logical slots (for example pagination page pills that slide across a window), updating only semantic state like `active` is not enough. Hover/focus/down animator state can leak from the previous slot occupant and make inactive items look selected. When a reused slot changes label/role or becomes inactive, explicitly cut/reset inner animator state (`hover.off`, `focus.off`, etc.) in addition to updating visibility or active flags.
